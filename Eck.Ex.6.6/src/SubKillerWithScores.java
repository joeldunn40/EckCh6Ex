/** Eck Exercise 6.6
 * Subkiller with scores (hits & misses)
 * hits is incremented in final frame of explsion animation
 * misses is incremented when bomb falls off canvas
 */
 ...
 */
public class SubKillerWithScores extends Application {
    ...
    private int misses = 0; 	// Number of Misses
    ...
    public void draw() {
        ...
        // Add text to keep score
        String hitsMessage = "Hits: " + hits;
        String missesMessage = "Misses: " + misses;
        g.setFont( Font.font(16) );
        g.fillText(hitsMessage,200,30);
        g.fillText(missesMessage,300,30);
        ...
        void updateForNewFrame() {  // If bomb is falling, take appropriate action.
            if (isFalling) {
                if (centerY > height) {
                    ...
                    isFalling = false;
                    misses++; // increment misses - this should only happen once...
                }
        ...
        void updateForNewFrame() { // Move sub or increase explosionFrameNumber.
            if (isExploding) {
                ...
                if (explosionFrameNumber == 25) { 
                    ...
                    isExploding = false;
                    isMovingLeft = (Math.random() < 0.5);
                    hits++; // increment hit score on final frame (i.e. once).
                }
            }
            else { // Move the sub.
                if (Math.random() < 0.02) {  
